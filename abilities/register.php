 ...
 * @return bool
 */
function dcp_user_can_manage_changesets() {
...
function dcp_ability_can_create_changeset( $input ) {
	return dcp_user_can_manage_changesets();
}
...
function dcp_ability_can_stage_content( $input ) {
	$source_id = isset( $input['source_post_id'] ) ? (int) $input['source_post_id'] : 0;
	return dcp_user_can_manage_changesets() && $source_id && current_user_can( 'edit_post', $source_id );
}
